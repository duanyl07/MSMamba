...
class LDA_SLIC(object):
    ...
    def SLIC_Process(self,img,scale=25,compactness=0.01):
        n_segments_init=self.height*self.width/scale
        print("n_segments_init",n_segments_init)   #0.01
        myslic=SLIC(img,n_segments=n_segments_init,labels=self.labes, compactness=compactness,sigma=1, min_size_factor=0.1, max_size_factor=2)
        Q, S, Segments = myslic.get_Q_and_S_and_Segments()
        A=myslic.get_A(sigma=10)
        return Q,S,A,Segments
        
    def simple_superpixel(self,scale,compactness):
        curr_labels = self.init_labels
        X = self.LDA_Process(curr_labels)
        Q, S, A, Seg = self.SLIC_Process(X,scale=scale,compactness=compactness)
        return Q, S, A,Seg
